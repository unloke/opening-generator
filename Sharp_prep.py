...
move_selection_iterations = 0  # 我方選步模擬回合數
branching_score_diff_threshold = 0  # 產生支線的評分差異閾值
wdl_depth = 10  # LC0 WDL 分析深度
...
def evaluate_score_from_info(info: dict, board: chess.Board, pov_is_white: bool) -> int:
    """
    以 CP 或 Mate 分值 (mate_score=100000) 換算成整數，以我方視角 (pov) 得出評分。
    """
...
# 取得 LC0 的 WDL 資訊
# ==============================
def get_lc0_wdl(lc0_engine: chess.engine.SimpleEngine, board: chess.Board, pov_is_white: bool) -> tuple:
    lc0_engine.configure({"UCI_ShowWDL": True})
    result = lc0_engine.analyse(board, chess.engine.Limit(depth=wdl_depth))
    wdl_obj = result.get("wdl")
    if wdl_obj is None:
        info = result.get("info", {})
        wdl_obj = info.get("wdl", None)
    if wdl_obj:
        # 若目前走棋方與我方一致，直接回傳；否則對調勝負值
        if board.turn == pov_is_white:
            return (wdl_obj[0], wdl_obj[1], wdl_obj[2])
        else:
            return (wdl_obj[2], wdl_obj[1], wdl_obj[0])
    return None
...
def select_best_move_for_us(
    ...
    lc0_engine_wdl: chess.engine.SimpleEngine,
    board: chess.Board,
    your_color: str,
    is_main_line: bool
) -> chess.Move:
    ...
    對每個候選步模擬多回合(Maia->SF)，最後用SF評估局面，並結合WDL計算「尖銳度」，
    最終依照 global 參數 `sharp` 來選擇步法。

    尖銳度 (Sharpness) = -log( Win_prob * Loss_prob )
      - Win_prob = w / (w + d + l)
      - Loss_prob = l / (w + d + l)
      - 若 Win_prob 或 Loss_prob = 0，則 -log(0) = +∞

    - sharp = 1：選出最「尖銳」 (越大越好)
    - sharp = 0：選出最「不尖銳」 (越小越好)
    """
    ...
    # (C) 逐一模擬
    import math
    results = []  # (move, final_score, sharpness_value)
    for move, initial_score in candidate_moves:
        ...
        # 計算尖銳度 (使用 LC0 的 WDL)
        sharpness_value = 0.0
        wdl_values = get_lc0_wdl(lc0_engine_wdl, temp_board, pov_is_white)
        if wdl_values:
            w, d, l = wdl_values
            total = w + d + l
            if total > 0:
                win_prob = w / float(total)
                loss_prob = l / float(total)
                # 檢查 win_prob 與 loss_prob 是否落在 (0, 1) 範圍內
                if win_prob <= 0 or win_prob >= 1 or loss_prob <= 0 or loss_prob >= 1:
                    sharpness_value = float('inf')
                    print(f"尖銳度計算錯誤：{wdl_values},局面:{temp_board.fen()}")
                else:
                    sharpness_value = 2 / (math.log(1 / win_prob - 1) + math.log(1 / loss_prob - 1))
                    print(f"尖銳度計算正確：{wdl_values},局面:{temp_board.fen()}")

        results.append((move, final_score, sharpness_value))

    # (D) 根據 sharp 選擇步法
    if sharp == 1:
        # 以 (sharpness_value, final_score) 做「同時最大化」排序
        best_move_data = max(results, key=lambda x: (x[2], x[1]))
    else:
        # 以 (sharpness_value, final_score) 做「同時最小化尖銳度、再看分數」的排序
        best_move_data = max(results, key=lambda x: (-x[2], x[1]))
...
def end_variation(board: chess.Board, node: chess.pgn.GameNode, your_color: str,
    ...
    if is_our_turn and not board.is_game_over():
        move = select_best_move_for_us(stockfish_engine, maia_engine, lc0_engine_wdl, board, your_color, is_main_line=False)
        if move:
            board.push(move)
...
def generate_variation(board: chess.Board,
    ...
    if board.is_game_over() or current_depth >= side_line_depth \
       or has_clear_advantage(stockfish_engine, board, your_color, base_score):
        end_variation(board, node, your_color, stockfish_engine, maia_engine, lc0_engine_wdl)
        return
...
def generate_opening_preparation(board: chess.Board,
    ...
    if is_our_turn:
        move = select_best_move_for_us(stockfish_engine, maia_engine, lc0_engine_wdl, board, your_color, is_main_line=True)
        if move is None:
            end_variation(board, node, your_color, stockfish_engine, maia_engine, lc0_engine_wdl)
            return
...
if __name__ == "__main__":
    ...
    lc0_command_wdl = [lc0_path, '--weights=D:\\吳冠頡\\ChessGPT\\lc0\\791556.pb.gz', '--multipv=1']

    # 在 Context Manager 中啟動並設定引擎
    with chess.engine.SimpleEngine.popen_uci(stockfish_path) as stockfish_engine:
        # 讓 Stockfish 回傳 WDL 資訊
        stockfish_engine.configure({"UCI_ShowWDL": True})

        with chess.engine.SimpleEngine.popen_uci(lc0_command) as maia_engine:
            with chess.engine.SimpleEngine.popen_uci(lc0_command_wdl) as lc0_engine_wdl:
                board = chess.Board()
                game = chess.pgn.Game()
