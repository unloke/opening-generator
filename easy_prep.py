...
move_selection_iterations = 8 # 我方選步模擬回合數 
...
# ==============================
# 「我方」選擇最佳步驟 (多次往返: Maia->Maia->Maia->Maia->... )
# ==============================
def select_best_move_for_us(
    ...
    """
    從 Stockfish 的多變化分析 (multipv) 中，挑出與最佳評分差距小的候選步。
    接著對每個候選步進行「Maia vs. Maia」的多次往返模擬，最後再用 Stockfish 評估最終局面，
    以選出最適合「我方」的走法。
    """
    ...
    # (C) 對所有候選步，進行「Maia vs. Maia」多次往返模擬，
    #     再用 Stockfish 評估最終局面，挑出分數最高者
    ...
    for move, initial_score in candidate_moves:
        ...
        current_score = initial_score  # 做為此候選步的起始分
        for _ in range(move_selection_iterations):
            ...
            # Maia 替「對手」走一步
            # --------------------------
            maia_info_opp = get_maia_analysis(maia_engine, temp_board)
            if not maia_info_opp or 'pv' not in maia_info_opp or not maia_info_opp['pv']:
                break

            opp_move = maia_info_opp['pv'][0]
            if opp_move not in temp_board.legal_moves:
                break
            ...
            # Maia 替「我方」再走一步
            # --------------------------
            maia_info_us = get_maia_analysis(maia_engine, temp_board)
            if not maia_info_us or 'pv' not in maia_info_us or not maia_info_us['pv']:
                break

            next_move = maia_info_us['pv'][0]
            if next_move not in temp_board.legal_moves:
                break
            temp_board.push(next_move)

        # 用 Stockfish 重新評估模擬後的最終局面
        if not temp_board.is_game_over():
            ...
            if final_infos and 'score' in final_infos[0]:
                ...
                final_score = current_score  # 若無法取得評分，就保留本次候選步最初的分數
        else:
